...
DoiHarvest是一个自动化工具，旨在帮助研究人员和学者批量下载学术论文PDF文件。它支持从Sci-Hub、Crossref、Unpaywall API获取论文，具有多线程下载、重试机制和智能域名轮询等功能，以提高下载效率和成功率。
...
- `table_utils.py`: 共享表格I/O调度（支持Excel、CSV、TXT读写）
...
5. 下载状态会自动更新到原始输入文件中
6. 运行`Crossref_download.py`或`Unpaywall_download.py`下载开放获取论文
   ```
